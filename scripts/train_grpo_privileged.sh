...
# sbatch lang_boot/scripts/train_grpo_privileged_gcs.sh \
...
#   -v /data/user_data/lsutawik/lbr-language_bootstrap_reasoning/ \

while getopts ":m:l:n:t:d:s:f:r:v:g:" opt; do
  case ${opt} in
    m ) MODEL=$OPTARG;;
    l ) LANGUAGE=$OPTARG;;
    n ) N_ROLLOUTS=$OPTARG;;
...
MAX_LENGTH=2048
RUN_NAME="${RUN_NAME:-grpo-privileged}"
RUN_NAME=${RUN_NAME}:${MODEL_ALIAS}:${TASK}:${LANGUAGE}:${N_ROLLOUTS}
